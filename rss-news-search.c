...
static void BuildIndices(const char *feedsFileName, hashset* stop_words, vector* word_info);
static void ProcessFeed(const char *remoteDocumentName, hashset* stop_words, vector* word_info);
static void PullAllNewsItems(FILE *dataStream, hashset* stop_words, vector* word_info);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stop_words, vector* word_info);
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL,
                         hashset* stop_words, vector* word_info);
static void QueryIndices(hashset* stop_words, vector* word_info);
static void ProcessResponse(const char *word, hashset* stop_words, vector* word_info);
...
const int NumBuckets = 1009;   
static const signed long kHashMultiplier = -1664117991L;
static const char *const kNewLineDelimiters = "\r\n";
static const char* const kStopListTextFile = "data/stop-words.txt";

int StrCmp(const void* elem_Addr1, const void* elem_Addr2){
  char* first_elem = *(char**)elem_Addr1;
  char* second_elem = *(char**)elem_Addr2;
  //printf("%s\n", first_elem);
  //printf("%s\n", second_elem);
  return strcasecmp(first_elem, second_elem);
}

void StrFree(void *elem_Addr1){
  char* elem = *(char**)elem_Addr1;
  free(elem);
}



int StrHash(const void* eleme_Addr1, int num_buckets){   
  char* str = *(char**)eleme_Addr1;
  unsigned long hash_code = 0;
  
  for (int j = 0; j < strlen(str); j++)  
    hash_code = hash_code * kHashMultiplier + tolower(str[j]);  
  return hash_code % num_buckets;                           
}


void ReadStopWords(hashset* stop_words){
  FILE *infile;
  streamtokenizer st;
  char tmp[1024];

  infile = fopen(kStopListTextFile, "r");
  assert(infile != NULL);

  STNew(&st, infile, kNewLineDelimiters, true);
  while (STNextToken(&st, tmp, sizeof(tmp))) {
    char* curr_word = strdup(tmp);
    HashSetEnter(stop_words, &curr_word);
  }
  STDispose(&st);
  fclose(infile);
}

void vectorFreeFn(void* elem_Addr){
  wordInfo* info = (wordInfo*)elem_Addr;
  free(*info -> word);
  free(info -> word);
  VectorDispose(&(info -> articleInfo));
}
...
int main(int argc, char **argv) {
  ...
  hashset stop_words;
  vector word_info;
  VectorNew(&word_info, sizeof(wordInfo), vectorFreeFn, 4);
  HashSetNew(&stop_words, sizeof(char**), NumBuckets, StrHash, StrCmp, StrFree);
  ReadStopWords(&stop_words);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stop_words, &word_info);
  QueryIndices(&stop_words, &word_info);
  curl_global_cleanup();
  HashSetDispose(&stop_words);
  VectorDispose(&word_info);
  return 0;
}
...
static void BuildIndices(const char *feedsFileName, hashset* stop_words, vector* word_info) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, stop_words, word_info);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset* stop_words, vector* word_info) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stop_words, word_info);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset* stop_words, vector* word_info) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stop_words, word_info);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stop_words, word_info);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset* stop_words, vector* word_info) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, stop_words, word_info);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stop_words, vector* word_info) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stop_words, word_info);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, stop_words, word_info);
  STDispose(&st);
  fclose(tmpDoc);
}


void vecFreeFn(void* elemAddr){
...
int vecSearchFn(const void* elemAddr1, const void* elemAddr2){
  article* articleInfo = (article*)elemAddr2;
  char* url1 = *(char**) elemAddr1;
  char* url2 = *articleInfo -> url;
  char* title1 = *((char**)elemAddr1 + 1);
  char* title2= *articleInfo -> title;
  int cmp_first = strcasecmp(url1, url2);
  if(cmp_first == 0) return cmp_first;
  int cmp_sec = strcasecmp(title1, title2);
  while(*url1 != '/'){
    url1++;
  }
  while(*url2 != '/'){
  ...
  int server_size = 0;
  while(url1[server_size] != '/' && url1[server_size] != '\0') server_size++;
  int cmp_th = strncasecmp(url1, url2, server_size);
  if(cmp_sec == 0 && cmp_th == 0) return 0;
  return cmp_first;
}
...
int searchCmp(const void* elem_Addr1, const void* elem_Addr2){
  char* first_elem = *(char**)elem_Addr1;
  wordInfo* info = (wordInfo*)elem_Addr2;
  return strcasecmp(first_elem, *info -> word); 
}


void addInfo(vector* word_info, const char* title, const char* URL, char* find_word){
  int index = VectorSearch(word_info, &find_word, searchCmp, 0, false);
  if(index == -1){
      wordInfo* new_one = malloc(sizeof(wordInfo));
      new_one -> word = malloc(sizeof(char**));
      *(new_one -> word) = strdup(find_word);
      VectorNew(&new_one -> articleInfo, sizeof(article), vecFreeFn, 4);
      article* new_article = malloc(sizeof(article));
      new_article -> url = malloc(sizeof(char**));
      *new_article -> url = strdup(URL);
      new_article -> title = malloc(sizeof(char**));
      *new_article -> title = strdup(title);
      new_article -> frequency = 1;
      VectorAppend(&(new_one -> articleInfo), new_article);
      VectorAppend(word_info, new_one);
      free(new_one);
      free(new_article);
  }else{
      wordInfo* elem = (wordInfo*)VectorNth(word_info, index);
      void* search_elem = malloc(2 * sizeof(char**));
      const char* url_ptr = URL;
      const char* title_ptr = title;
      memcpy(search_elem, &url_ptr, sizeof(char**));
      memcpy((char*)search_elem + sizeof(char**), &title_ptr, sizeof(char**));
      int search_ind = VectorSearch(&(elem -> articleInfo), search_elem, vecSearchFn, 0, false);
      free(search_elem);
      if(search_ind == -1){
        article* new_article = malloc(sizeof(article));
        new_article -> url = malloc(sizeof(char**));
        ...
        *new_article -> title = strdup(title);
        new_article -> frequency =  1;
        VectorAppend(&elem -> articleInfo, new_article);
        free(new_article);
      }else{
        article* nth = (article*)VectorNth(&elem -> articleInfo, search_ind);
        nth -> frequency++;
      }
  }
}
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        numWords++;
        char* find_word = word;
        if(HashSetLookup(stop_words, &find_word) == NULL){
          addInfo(word_info, articleTitle, articleURL, find_word);
        }
...
static void QueryIndices(hashset* stop_words, vector* word_info) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stop_words, word_info);
  }
}

int sortByfrequency(const void* elem_Addr1, const void* elem_Addr2){
    article* article1 = (article*)elem_Addr1;
    article* article2 = (article*)elem_Addr2;
    return -(article1 -> frequency - article2 -> frequency);
}
...
static void ProcessResponse(const char *word, hashset* stop_words, vector* word_info) {
  if (WordIsWellFormed(word)) {
    int search_ind = VectorSearch(word_info, &word, searchCmp, 0, false);
    if(search_ind == -1){ 
      ...
      wordInfo* curr_word = (wordInfo*)VectorNth(word_info, search_ind);
      VectorSort(&curr_word -> articleInfo, sortByfrequency);
      for(int k = 0; k < VectorLength(&curr_word -> articleInfo); k++){
        article* tmp = VectorNth(&curr_word -> articleInfo, k);
        int frequency = tmp -> frequency;
        if(frequency == 1){
          printf("%d.) \"%s\" [search term occurs %d time]\n", k + 1, *tmp -> title, frequency);
          printf("\"%s\"\n", *tmp -> url);
        }else{
